...
- **Half-open intervals:** a rule is in force on D when `valid_from <= D < valid_to`; missing `valid_from` → `date.min`, missing `valid_to` → `date.max` (`reconcile.py:589-615`, defaults at 611-613). Therefore *close old at D + open new at D* is gapless and overlap-free by construction; on D itself the **new** rule is in force. **Since 2026-09-14 the reconciler evaluates every invoice line on its run date (`reconcile_lines(as_of=today)`), not the invoice date** — the windows record history and drive the grid, list view and export; a master correction is verified by re-uploading the weekly file.
- **Overlaps are legal and load-bearing:** `_index_rules` sorts in precedence order and `_lookup_rule` returns the first containing window. **Precedence is status-aware** (`reconcile.rule_precedence`, 2026-09-02): an in-window `supported` rule wins over the standing rule (`tenanted`/`managed`) whatever their `valid_from` dates; within a tier newest `valid_from` first. `/add-support` deliberately layers a bounded `supported` rule over the still-open standard rule with `close_keys_at_date=None`. Plain newest-first let every cost change (which re-dates the standing rule to today) silently cancel an active support — hence the tiering. The same selection (`reconcile.select_winners` / `winning_rule`) drives the pivot, the list view's "wins on" pill, the previews (`_winner_on`) and the export. **Do not add "no overlaps ever" validation** — it would break the support mechanism. Instead, show which rule *wins* on any date.
- **Supports survive cost changes:** every cost path (grid Price/Retro cells, product settings, `set_product_retro`, `/upload-master`, the annual increase) rewrites an active support's `fb_price`/`retro_pct` **in place under its own `rule_key`** (`reconcile.support_cost_patch`) — tenant price and window untouched — so `wrong_fb_price` stays right inside the window. Never re-date a support: `site|product|today` is the tenanted successor's key. A `supported` row in an upsert batch never triggers the close pass (`airtable_io.upsert_pricing_rules`, mirrored by `preview_master_change` / `patch_snapshot_for_*`). A grid-cell or findings-page tenant-price edit at a supported site is a fix-in-place of the support; `validate_master_change` warns when a standing change is written under an active support.
- **One cost successor per (site, product):** the cost paths build their today-dated successor from `reconcile.successor_bases` (the winning open tenanted rule per key on the effective date), never one per row — two open rows for one key (legacy `|open` beside a dated one, or a half-landed prior upsert's original beside its successor) would otherwise create two rows sharing the successor's `rule_key`. Stragglers are ended by the close pass; the annual increase re-saves an already-at-date successor unchanged (a "heal") purely to finish that close.
...
|---|---|---|---|
| **Change price from date D** | The price genuinely changed | `upsert_pricing_rules([new_rule], close_keys_at_date=D)` — close pass PATCHes the prior open rule's `valid_to=D` (scoped to this (site,product) via `keys_in_new`); new row created with `valid_from=D`, `valid_to=None`. Re-submitting the same D updates in place (same-key belt). A `status=supported` rule closes **nothing** — it layers over the standing rule, which resumes after the support's window. | Preserved |
...
Note the caveat for retro-corrections: already-persisted `Mismatches` rows are **not** recomputed by the edit itself — re-uploading the affected weekly file is what brings them in line. `Files` dedupes by `raw_hash`, so the re-run lands on the same file record; `write_mismatches` keys each finding on the line's sheet row (stable across runs) and runs a per-file sync (`airtable_io._sync_file_findings`, 2026-09-14): findings the fix removes are marked `status=superseded` with a dated note (never deleted), survivors keep their rows with figures refreshed and operator status/notes untouched, and nothing is duplicated. The confirm page says so.
...
8. Overlaps allowed; the preview must state which rule wins on the effective date (an in-window `supported` rule first, then newest `valid_from` — `reconcile.rule_precedence`), and warn when an active support keeps winning over a standing change. Gaps allowed but warned (§2.1 membership).
