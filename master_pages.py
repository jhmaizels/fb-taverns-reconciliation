...
var MG=""" + f"{MARGIN_GREEN_ABOVE_PCT:g},MR={MARGIN_RED_BELOW_PCT:g}" + r""";
...
function updMargins(tr){var fbi=tr.querySelector('input[name=fb_price]');if(!fbi)return;var fb=parseFloat(fbi.value);if(isNaN(fb))return;var ri=tr.querySelector('input[name=retro_gbp]');var net=fb-(ri?(parseFloat(ri.value)||0):0);Array.prototype.forEach.call(tr.querySelectorAll('input[name=tenant_price]'),function(ti){var td=ti.closest('td');var m=td?td.querySelector('.cell-margin'):null;var tp=parseFloat(ti.value);if(!m||isNaN(tp))return;var mg=tp-net;var pct=tp>0?(mg/tp*100):null;var cls=pct===null?'':(pct<MR?'cell-neg':(pct>MG?'cell-pos':'cell-warn'));m.className='cell-margin '+cls;m.innerHTML=fmt(mg)+'<span class="pct">'+(pct===null?'n/a':pct.toFixed(1)+'%')+'</span>';});}
function updNet(i){var tr=i.closest('tr');if(!tr)return;var n=tr.querySelector('.net-val');var fbi=tr.querySelector('input[name=fb_price]');if(!n||!fbi)return;if(n._orig===undefined)n._orig=n.innerHTML;var fb=parseFloat(fbi.value);if(isNaN(fb)){n.innerHTML=n._orig;return;}var ri=tr.querySelector('input[name=retro_gbp]');var r=ri?(parseFloat(ri.value)||0):0;n.textContent=fmt(fb-r);updMargins(tr);}
t.addEventListener('input',function(e){var i=e.target;if(!i||!i.classList||!i.classList.contains('cell-input'))return;refresh();if(i.name==='fb_price'||i.name==='retro_gbp')updNet(i);else if(i.name==='tenant_price'){var tr=i.closest('tr');if(tr)updMargins(tr);}});
